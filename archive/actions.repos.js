...
composer.action(`repos`, async (ctx) => {
  ...
  const keyboard = Object.values(fetch).map((obj) => [
    Markup.callbackButton(`${obj["name"]}`, `repo_${obj["name"]}`),
  ]);
...
composer.action(/repo_(.*)/gi, async (ctx) => {
  ...
  await ctx.editMessageMedia(
    { caption: text, type: "photo", media: { source: `./assets/repo.png` } },
    {
      parse_mode: "HTML",
